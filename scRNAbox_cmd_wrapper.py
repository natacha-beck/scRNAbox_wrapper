...
parser.add_argument('--par_automated_library_prep', action='store_true', help='If you want to perform automated library preparation')
...
parser.add_argument('--par_fastq_directory', type=str, help='Directory containing the FASTQ files for the experiment')

## List the sample names used in the FASTQ nomenclature
#par_sample_names= c("Sample1", "Sample2", "Sample3")
parser.add_argument('--step1_par_sample_names', type=str, help='List the sample names used in the FASTQ nomenclature (step 1)')

## If you want to rename the samples, set par_rename_samples to yes.
#par_rename_samples= "yes"
parser.add_argument('--par_rename_samples', action='store_true', help='If you want to rename the samples')
...
parser.add_argument('--par_new_sample_names', type=str, help='List the new sample names in the same order as the old labels are listed in par_sample_names')

## If your sequencing is paired-end, set the following to TRUE. Otherwise set it as FALSE.
#par_paired_end_seq=TRUE
parser.add_argument('--par_paired_end_seq', action='store_true', help='If your sequencing is paired-end')

# CellRanger counts pipeline parameters.
## Path to reference genome
#par_ref_dir_grch='/path/to/CellRanger/reference/genome'
parser.add_argument('--par_ref_dir_grch', type=str, help='Reference genome')
...
#par_r1_length=20
parser.add_argument('--par_r1_length', type=int, help='Minimum number of bases to retain for R1 sequence of gene expression assay')
...
#par_r2_length=20
parser.add_argument('--par_r2_length', type=int, help='Minimum number of bases to retain for R2 sequence of gene expression assay')
...
#par_mempercode=30
parser.add_argument('--par_mempercode', type=int, help='For clusters whose job managers do not support memory requests, it is possible to request memory in the form of cores')
...
parser.add_argument('--par_include_introns', action='store_true', help='If you want CellRnager to include introns when producing the gene expression matrices')
...
parser.add_argument('--par_no_target_umi_filter', action='store_true', help='If you want to turn off CellRanger\'s target UMI filtering subpipeline')
...
parser.add_argument('--par_expect_cells', type=int, help='If you want to specify the number of expected cells')
...
parser.add_argument('--par_force_cells', type=int, help='If you want to force the CellRanger count pipeline to use a certain number of cells')

## If you want to skip the bam file generation, uncomment the parameter below.
#par_no_bam="no"
parser.add_argument('--par_no_bam', action='store_true', help='If you want to skip the bam file generation')
...
parser.add_argument('--step2_par_save_RNA', action='store_true', help='If you want to save an RNA expression matrix (step 2)')
#step2_par_save_metadata= "yes"
parser.add_argument('--step2_par_save_metadata', action='store_true', help='If you want to save a metadata dataframe (step 2)')
...
# par_ambient_RNA= "yes"
parser.add_argument('--par_ambient_RNA', action='store_true', help='If you want to remove ambient RNA from the expression matrix')

# Filtering parameters
############################################################################
## Only retain genes that are present in at least a specified number of cells.
# par_min.cells_L= 3
parser.add_argument('--par_min_cells_L', type=int, help='Only retain genes that are present in at least a specified number of cells')
...
parser.add_argument('--step2_par_normalization_method', type=str, help='Normalization method (step 2)')

## Scale factor
# par_scale.factor= 10000
parser.add_argument('--step2_par_scale_factor', type=int, help='Scale factor (step 2)')

## Method for choosing the top variable features. vst, mean.var.plot (mvp), dispersion (disp).
# par_selection.method= "vst"
parser.add_argument('--step2_par_selection_method', type=str, help='Method for choosing the top variable features (step 2)')

## Number of features to select as top variable features
# par_nfeatures= 2500
parser.add_argument('--step2_par_nfeatures', type=int, help='Number of features to select as top variable features (step 2)')
...
############################################################################
# par_save_RNA= "yes"
parser.add_argument('--step3_par_save_RNA', action='store_true', help='If you want to save an RNA expression matrix (step 3)')
# par_save_metadata= "yes"
parser.add_argument('--step3_par_save_metadata', action='store_true', help='If you want to save a metadata dataframe (step 3)')
...
parser.add_argument('--step3_par_seurat_object', type=str, help='Directory containing Seurat object(s) (step 3)')
...
parser.add_argument('--par_nFeature_RNA_L', type=int, help='Minimum number of unique RNA transcripts')

## Maximum number of unique RNA transcripts
# par_nFeature_RNA_U= 10000
parser.add_argument('--par_nFeature_RNA_U', type=int, help='Maximum number of unique RNA transcripts')

## Minimum number of total RNA transcripts
# par_nCount_RNA_L= 300
parser.add_argument('--par_nCount_RNA_L', type=int, help='Minimum number of total RNA transcripts')

## Maximum number of total RNA transcripts
# par_nCount_RNA_U= 20000
parser.add_argument('--par_nCount_RNA_U', type=int, help='Maximum number of total RNA transcripts')

## Minimum mitochondrial RNA percentage
# par_mitochondria_percent_L= 0
parser.add_argument('--par_mitochondria_percent_L', type=int, help='Minimum mitochondrial RNA percentage')

## Maximum mitochondrial RNA percentage
# par_mitochondria_percent_U= 20
parser.add_argument('--par_mitochondria_percent_U', type=int, help='Maximum mitochondrial RNA percentage')

## Minimum ribosomal RNA percentage
# par_ribosomal_percent_L= 0
parser.add_argument('--par_ribosomal_percent_L', type=int, help='Minimum ribosomal RNA percentage')

## Maximum ribosomal RNA percentage
# par_ribosomal_percent_U= 100
parser.add_argument('--par_ribosomal_percent_U', type=int, help='Maximum ribosomal RNA percentage')
...
parser.add_argument('--par_remove_mitochondrial_genes', action='store_true', help='If you want to filter out mitochondrial genes')
# par_remove_ribosomal_genes= "no"
parser.add_argument('--par_remove_ribosomal_genes', action='store_true', help='If you want to filter out ribosomal genes')
...
parser.add_argument('--par_remove_genes', type=str, help='List of genes to remove')
...
parser.add_argument('--par_regress_cell_cycle_genes', action='store_true', help='If you want to regress cell cycle genes')
...
parser.add_argument('--par_regress_custom_genes', action='store_true', help='If you want to regress a custom list of genes')

## Enter the genes that you want to regress in the list below.
# par_regress_genes= c("gene1", "gene2")
parser.add_argument('--par_regress_genes', type=str, help='List of genes to regress')

# Parameters for normalization and scaling after quality control
############################################################################
## Normalization method
# par_normalization.method= "LogNormalize"
parser.add_argument('--step3_par_normalization_method', type=str, help='Normalization method (step 3)')

## Scale factor
# par_scale.factor= 10000
parser.add_argument('--step3_par_scale_factor', type=int, help='Scale factor (step 3)')

## Method for choosing the top variable features. vst, mean.var.plot (mvp), dispersion (disp).
# par_selection.method= "vst"
parser.add_argument('--step3_par_selection_method', type=str, help='Method for choosing the top variable features (step 3)')

## Number of features to select as top variable features
# par_nfeatures= 2500
parser.add_argument('--step3_par_nfeatures', type=int, help='Number of features to select as top variable features (step 3)')

## Number of most variable features to be reported in csv file
# par_top= 10
parser.add_argument('--par_top', type=int, help='Number of most variable features to be reported in csv file')

## Total Number of PCs to compute and store for RunPCA
# par_npcs_pca= 30
parser.add_argument('--par_npcs_pca', type=int, help='Total Number of PCs to compute and store for RunPCA')
...
############################################################################
# par_save_RNA= "yes"
parser.add_argument('--step4_par_save_RNA', action='store_true', help='If you want to save an RNA expression matrix (step 4)')
# par_save_metadata= "yes"
parser.add_argument('--step4_par_save_metadata', action='store_true', help='If you want to save a metadata dataframe (step 4)')
...
############################################################################
#par_seurat_object= "/path/to/directory/containing/seurat/object"
parser.add_argument('--step4_par_seurat_object', type=str, help='Directory containing Seurat object(s) (step 4)')

# Parameters for UMAP dimensional reduction
############################################################################
## Number of dimensions to use as input into UMAP
# par_RunUMAP_dims= 25
parser.add_argument('--step4_par_RunUMAP_dims', type=int, help='Number of dimensions to use as input into UMAP (step 4)')

## Number of neighbouring points to use in local approximation of manifold structure
# par_RunUMAP_n.neighbors= 45
parser.add_argument('--step4_par_RunUMAP_n_neighbors', type=int, help='Number of neighbouring points to use in local approximation of manifold structure (step 4)')
...
# par_dropDN= "yes"
parser.add_argument('--par_dropDN', action='store_true', help='If you want to remove predicted doublets from downstream analyses')
...
# par_PCs= 25
parser.add_argument('--par_PCs', type=int, help='Number of principal components to use as input doublet analysis')
...
# par_pN= 0.25
parser.add_argument('--par_pN', type=float, help='The number of artificial doublets to generate')

## Logical representing whether SCTransform was used during original Seurat object pre-processing
# par_sct= FALSE
parser.add_argument('--par_sct', action='store_true', help='Logical representing whether SCTransform was used during original Seurat object pre-processing')

##rate_nExp: the doublet rate according to the number of cells
# par_rate_nExp=0.076
parser.add_argument('--par_rate_nExp', type=float, help='The doublet rate according to the number of cells')
...
parser.add_argument('--step4_par_sample_names', type=str, help='List of sample names (step 4)')
# par_expected_doublet_rate= c(0.05,0.05)
parser.add_argument('--par_expected_doublet_rate', type=str, help='List of expected doublet rates')
...
############################################################################
# par_save_RNA= "yes"
parser.add_argument('--step5_par_save_RNA', action='store_true', help='If you want to save an RNA expression matrix (step 5)')
# par_save_metadata= "yes"
parser.add_argument('--step5_par_save_metadata', action='store_true', help='If you want to save a metadata dataframe (step 5)')
...
parser.add_argument('--step5_par_seurat_object', type=str, help='Directory containing Seurat object(s) (step 5)')

# If you only have one Seurat object and want to skip integration set the following to "yes"
############################################################################
# par_one_seurat= "no"
parser.add_argument('--par_one_seurat', action='store_true', help='If you only have one Seurat object and want to skip integration')
...
parser.add_argument('--par_integrate_seurat', action='store_true', help='If you want to integrate Seurat objects')

## Merge Seurat objects
# par_merge_seurat= "no"
parser.add_argument('--par_merge_seurat', action='store_true', help='If you want to merge Seurat objects')
...
parser.add_argument('--par_DefaultAssay', type=str, help='Assay to perform normalization and scaling')

## Normalization method
# par_normalization.method= "LogNormalize"
parser.add_argument('--step5_par_normalization_method', type=str, help='Normalization method')

## Scale factor
# par_scale.factor= 10000
parser.add_argument('--step5_par_scale_factor', type=int, help='Scale factor')

# Parameters for integration
############################################################################
## Method for detecting top variable features. vst, mean.var.plot (mvp), dispersion (disp)
# par_selection.method= "vst"
parser.add_argument('--step5_par_selection_method', type=str, help='Method for detecting top variable features (step 5)')

## Number of features to select as top variable features for integration
# par_nfeatures= 2500
parser.add_argument('--step5_par_nfeatures', type=int, help='Number of features to select as top variable features for integration (step 5)')

## Which dimensions to use from the CCA to specify the neighbour search space
# par_FindIntegrationAnchors_dim= 25
parser.add_argument('--par_FindIntegrationAnchors_dim', type=int, help='Which dimensions to use from the CCA to specify the neighbour search space')
...
## Total Number of PCs to compute and store for RunPCA
# par_RunPCA_npcs= 30
parser.add_argument('--par_RunPCA_npcs', type=int, help='Total Number of PCs to compute and store for RunPCA')

## Which dimensions to use as input features for RunUMAP
# par_RunUMAP_dims= 25
parser.add_argument('--step5_par_RunUMAP_dims', type=int, help='Which dimensions to use as input features for RunUMAP (step 5)')

## The number of neighbouring points used in local approximations of manifold structure.
# par_RunUMAP_n.neighbors= 45
parser.add_argument('--step5_par_RunUMAP_n_neighbors', type=int, help='The number of neighbouring points used in local approximations of manifold structure (step 5)')

## Whether or not to perform JackStraw computation. This computation takes a long time.
# par_compute_jackstraw= "no"
parser.add_argument('--par_compute_jackstraw', action='store_true', help='Whether or not to perform JackStraw computation')
...
############################################################################
# par_save_RNA= "yes"
parser.add_argument('--step6_par_save_RNA', action='store_true', help='If you want to save an RNA expression matrix (step 6)')
# par_save_metadata= "yes"
parser.add_argument('--step6_par_save_metadata', action='store_true', help='If you want to save a metadata dataframe (step 6)')
...
parser.add_argument('--step6_par_seurat_object', type=str, help='Path to Seurat object (step 6)')
...
parser.add_argument('--par_skip_integration', action='store_true', help='If you skipped integration in step 5')
...
parser.add_argument('--par_FindNeighbors_dims', type=int, help='Number of PC to use as input to find neighbours')

## Number of PCs to use as input for UMAP. Can be informed by the elbow and jackstraw plots produced in Step 5.
# par_RunUMAP_dims= 25
parser.add_argument('--step6_par_RunUMAP_dims', type=int, help='Number of PCs to use as input for UMAP')
...
parser.add_argument('--par_FindNeighbors_k_param', type=int, help='Defines k for the k-nearest neighbour algorithm')
...
parser.add_argument('--par_FindNeighbors_prune_SNN', type=float, help='Sets the cutoff for acceptable Jaccard index when computing the neighbourhood overlap for the SNN construction')
...
parser.add_argument('--par_FindClusters_resolution', type=str, help='Value of the clustering resolution parameter')

# Adjusted Rand Index for clustering resolutions
############################################################################
## If you want to compute ARI, set the following to "yes". This takes a long time.
# par_compute_ARI= "yes"
parser.add_argument('--par_compute_ARI', action='store_true', help='If you want to compute ARI')

## Number of repetitions of generated clusters to calculate ARI.
# par_RI_reps= 25
parser.add_argument('--par_RI_reps', type=int, help='Number of repetitions of generated clusters to calculate ARI')
...
############################################################################
# par_save_RNA= "yes"
parser.add_argument('--step7_par_save_RNA', action='store_true', help='If you want to save an RNA expression matrix (step 7)')
# par_save_metadata= "yes"
parser.add_argument('--step7_par_save_metadata', action='store_true', help='If you want to save a metadata dataframe (step 7)')
...
############################################################################
#par_seurat_object= "/path/to/seurat.rds"
parser.add_argument('--step7_par_seurat_object', type=str, help='Path to Seurat object (step 7)')
...
parser.add_argument('--par_level_cluster', type=str, help='The cluster resolution that you want to use')

# Tool 1: Marker GSEA
############################################################################
## Identify cluster specific markers
# par_run_find_marker= "yes"
parser.add_argument('--par_run_find_marker', action='store_true', help='If you want to identify cluster specific markers')
...
# par_run_enrichR= "no"
parser.add_argument('--par_run_enrichR', action='store_true', help='If you want to run EnrichR GSEA on cluster-specific markers')

## Number of top markers based on avg_log2FC
## This is the number of markers to include on a heatmap for visualization.
# par_top_sel= 5
parser.add_argument('--par_top_sel', type=int, help='Number of top markers based on avg_log2FC')
...
parser.add_argument('--par_db', type=str, help='Character vector of EnrichR databases to search for enrichment')

# Tool 2: Visualize the expression of known marker genes
######################################################################
## Perform module score computation
# par_run_module_score= "yes"
parser.add_argument('--par_run_module_score', action='store_true', help='If you want to perform module score computation')

## Visualize markers (dot plot, violin plot, feature plot)
# par_run_visualize_markers= "yes"
parser.add_argument('--par_run_visualize_markers', action='store_true', help='If you want to visualize markers')

## Define the path to a csv file containing the genes sets for module score
# par_module_score= "/path/to/gene_sets.csv"
parser.add_argument('--par_module_score', type=str, help='csv file containing the genes sets for module score')
...
parser.add_argument('--par_select_features_list', type=str, help='List of markers that you want to visualize')
...
parser.add_argument('--par_select_features_csv', type=str, help='csv file containing the markers that you want to visualize')
...
parser.add_argument('--par_reference', type=str, help='Seurat RDS object to use as the reference')
...
parser.add_argument('--par_reference_name', type=str, help='Define an arbitrary name for the reference object')

## Name of a metadata column in the reference Seurat object that contains cell type annotations
# par_level_celltype= "Cell_Type"
parser.add_argument('--par_level_celltype', type=str, help='Name of a metadata column in the reference Seurat object that contains cell type annotations')

## How many dimensions to use to find transfer anchors between query and reference dataset
# par_FindTransferAnchors_dim= 50
parser.add_argument('--par_FindTransferAnchors_dim', type=int, help='How many dimensions to use to find transfer anchors between query and reference dataset')

## This will increase your RAM usage so set this number mindfully
# par_futureglobalsmaxSize= 60000 * 1024^2
parser.add_argument('--par_futureglobalsmaxSize', type=int, help='This will increase your RAM usage so set this number mindfully')
...
parser.add_argument('--par_annotate_resolution', type=str, help='The clustering resolution to annotate')

## the name of the metadata slot under which the cluster labels will be stored.
# par_name_metadata= "Celltypes1"
parser.add_argument('--par_name_metadata', type=str, help='The name of the metadata slot under which the cluster labels will be stored')
...
parser.add_argument('--par_annotate_labels', type=str, help='A list of cluster labels')
...
############################################################################
# par_save_RNA= "yes"
parser.add_argument('--step8_par_save_RNA', action='store_true', help='If you want to save an RNA expression matrix (step 8)')
# par_save_metadata= "yes"
parser.add_argument('--step8_par_save_metadata', action='store_true', help='If you want to save a metadata dataframe (step 8)')
...
############################################################################
#par_seurat_object= "/path/to/seurat.rds"
parser.add_argument('--step8_par_seurat_object', type=str, help='Path to Seurat object (step 8)')

# Add metadata parameters
############################################################################
## Define the column from the Seurat object that you want to use to add the new metadata
# par_merge_meta= "Sample_ID"
parser.add_argument('--par_merge_meta', type=str, help='Column from the Seurat object that you want to use to add the new metadata')
...
parser.add_argument('--par_metadata', type=str, help='Path to a csv file describing new metadata that should be added to the Seurat object')
...
parser.add_argument('--par_run_cell_based_all_cells', action='store_true', help='If you want to perform cell-based DGE with all cells')

## Perform cell-based on each cell type group
# par_run_cell_based_celltype_groups= "yes"
parser.add_argument('--par_run_cell_based_celltype_groups', action='store_true', help='If you want to perform cell-based DGE on each cell type group')

## Perform Sample-based DGE with all cells (pseudobulk)
# par_run_sample_based_all_cells= "yes"
parser.add_argument('--par_run_sample_based_all_cells', action='store_true', help='If you want to perform sample-based DGE with all cells')

## Perform Sample-based DGE on each cell type group (pseudobulk)
# par_run_sample_based_celltype_groups= "yes"
parser.add_argument('--par_run_sample_based_celltype_groups', action='store_true', help='If you want to perform sample-based DGE on each cell type group')
...
parser.add_argument('--par_statistical_method', type=str, help='Which statistical method to use when computing DGE using individual cells as replicates')
...
if args.step1_par_sample_names:
    step1_file.write("par_sample_names= c(" + args.step1_par_sample_names + ")\n")
...
if args.step2_par_save_RNA:
    step2_file.write("step2_par_save_RNA=\"yes\"\n")

if args.step2_par_save_metadata:
    step2_file.write("step2_par_save_metadata=\"yes\"\n")
...
if args.step2_par_normalization_method:
    step2_file.write("par_normalization_method= " + args.step2_par_normalization_method + "\n")

if args.step2_par_scale_factor:
    step2_file.write("par_scale_factor=" + str(args.step2_par_scale_factor) + "\n")

if args.step2_par_selection_method:
    step2_file.write("par_selection_method= " + args.step2_par_selection_method + "\n")

if args.step2_par_nfeatures:
    step2_file.write("par_nfeatures=" + str(args.step2_par_nfeatures) + "\n")
...
if args.step3_par_save_RNA:
    step3_file.write("step3_par_save_RNA=\"yes\"\n")

if args.step3_par_save_metadata:
    step3_file.write("step3_par_save_metadata=\"yes\"\n")

if args.step3_par_seurat_object:
    step3_file.write("step3_par_seurat_object= " + args.step3_par_seurat_object + "\n")
...
if args.step4_par_save_RNA:
    step4_file.write("step4_par_save_RNA=\"yes\"\n")

if args.step4_par_save_metadata:
    step4_file.write("step4_par_save_metadata=\"yes\"\n")

if args.step4_par_seurat_object:
    step4_file.write("step4_par_seurat_object= " + args.step4_par_seurat_object + "\n")

if args.step4_par_RunUMAP_dims:
    step4_file.write("par_RunUMAP_dims=" + str(args.step4_par_RunUMAP_dims) + "\n")

if args.step4_par_RunUMAP_n_neighbors:
    step4_file.write("par_RunUMAP_n_neighbors=" + str(args.step4_par_RunUMAP_n_neighbors) + "\n")
...
if args.step4_par_sample_names:
    step4_file.write("par_sample_names= c(" + args.step4_par_sample_names + ")\n")
...
if args.step5_par_save_RNA:
    step5_file.write("step5_par_save_RNA=\"yes\"\n")

if args.step5_par_save_metadata:
    step5_file.write("step5_par_save_metadata=\"yes\"\n")

if args.step5_par_seurat_object:
    step5_file.write("step5_par_seurat_object= " + args.step5_par_seurat_object + "\n")
...
if args.step5_par_normalization_method:
    step5_file.write("par_normalization_method= " + args.step5_par_normalization_method + "\n")

if args.step5_par_scale_factor:
    step5_file.write("par_scale_factor=" + str(args.step5_par_scale_factor) + "\n")

if args.step5_par_selection_method:
    step5_file.write("par_selection_method= " + args.step5_par_selection_method + "\n")

if args.step5_par_nfeatures:
    step5_file.write("par_nfeatures=" + str(args.step5_par_nfeatures) + "\n")
...
if args.step5_par_RunUMAP_dims:
    step5_file.write("par_RunUMAP_dims=" + str(args.step5_par_RunUMAP_dims) + "\n")

if args.step5_par_RunUMAP_n_neighbors:
    step5_file.write("par_RunUMAP_n_neighbors=" + str(args.step5_par_RunUMAP_n_neighbors) + "\n")
...
if args.step6_par_save_RNA:
    step6_file.write("step6_par_save_RNA=\"yes\"\n")

if args.step6_par_save_metadata:
    step6_file.write("step6_par_save_metadata=\"yes\"\n")

if args.step6_par_seurat_object:
    step6_file.write("step6_par_seurat_object= " + args.step6_par_seurat_object + "\n")
...
if args.step6_par_RunUMAP_dims:
    step6_file.write("par_RunUMAP_dims=" + str(args.step6_par_RunUMAP_dims) + "\n")
...
if args.step7_par_save_RNA:
    step7_file.write("step7_par_save_RNA=\"yes\"\n")

if args.step7_par_save_metadata:
    step7_file.write("step7_par_save_metadata=\"yes\"\n")

if args.step7_par_seurat_object:
    step7_file.write("step7_par_seurat_object= " + args.step7_par_seurat_object + "\n")
...
if args.step8_par_save_RNA:
    step8_file.write("step8_par_save_RNA=\"yes\"\n")

if args.step8_par_save_metadata:
    step8_file.write("step8_par_save_metadata=\"yes\"\n")

if args.step8_par_seurat_object:
    step8_file.write("step8_par_seurat_object= " + args.step8_par_seurat_object + "\n")
