...
## Install Matrix      ##
#########################
write("Installation of packages Matrix", stdout())

install.packages("Matrix", version = "1.6.3", repos='https://cran.r-project.org', dependencies=TRUE)
if (! library("Matrix", character.only = TRUE, logical.return = TRUE)) {
  write(paste0("Installation of package ", "Matrix" ," exited with non-zero exit status"),
         stdout())
  quit(status = 1, save = "no")
}

pkgs_cran<-c('jquerylib', 'sass', 'fontawesome', 'bslib', 'rmarkdown')
...
## Install BiocManager ##
#########################
write("Installation of BiocManager", stdout())

install.packages("BiocManager",repos='https://cran.r-project.org', dependencies=TRUE)

if (! library("BiocManager", character.only = TRUE, logical.return = TRUE)) {
  write(paste0("Installation of package ", "BiocManager" ," exited with non-zero exit status"),
        stdout())
  quit(status = 1, save = "no")
}
...
## Install multtest ##
######################
write("Installation of multtest", stdout())

BiocManager::install("multtest")

if (! library("multtest", character.only = TRUE, logical.return = TRUE)) {
  write(paste0("Installation of package ", "multtest" ," exited with non-zero exit status"),
        stdout())
  quit(status = 1, save = "no")
}
...
pkgs_cran<-c('beeswarm','pdftools','magick','callr','pkgbuild','diffobj','reprex','rvest','RSpectra','uwot',
	     'broom', 'dbplyr', 'modelr', 'tidyr',
             'tidyverse', 'foreach', 'doParallel', 'Seurat', 'caTools','gplots','ROCR','vctrs',
             'fossil', 'openxlsx', 'stringr','ggsignif','ggpubr','data.table','ggrepel', 'scCustomize',
             'fs', 'bslib', 'pkgload', 'shiny', 'gargle', 'htmlwidgets', 'testthat',
             'usethis', 'miniUI', 'pkgdown', 'profvis', 'roxygen2', 'mutoss', 'RcppGSL', 'RcppZiggurat',
             'igraph', 'Rfast','leiden','reticulate')
...
###########################
write("Installation of DoubletFinder", stdout())
...
pkgs_BiocManager <- c('cowplot','xlsx','clustree','enrichR','stringi','limma', 'edgeR',
...
## Install scrnabox ##
######################
write("Installation of package scrnaboxR",  stdout())
